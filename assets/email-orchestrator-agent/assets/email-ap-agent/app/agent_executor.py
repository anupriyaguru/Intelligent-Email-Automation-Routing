"""Agent executor — wires the A2A protocol to the agent."""
import logging
...
class AgentExecutor(A2AAgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        user_token = [REDACTED]        updater = TaskUpdater(event_queue, task.id, task.context_id)

        try:
            try:
                mcp_tools = await get_mcp_tools(user_token=[REDACTED] or []
            except Exception:
